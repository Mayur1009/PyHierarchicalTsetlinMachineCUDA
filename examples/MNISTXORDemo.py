...
for i in range(number_of_training_examples):
	...
	X_train[i,:28*28] = X_mnist_train[Y_mnist_train == x[0]][np.random.randint(X_mnist_train[Y_mnist_train == x[0]].shape[0])]
	X_train[i,28*28:] = X_mnist_train[Y_mnist_train == x[1]][np.random.randint(X_mnist_train[Y_mnist_train == x[1]].shape[0])]	
...
for i in range(number_of_testing_examples):
	...
	X_test[i,:28*28] = X_mnist_train[Y_mnist_train == x[0]][np.random.randint(X_mnist_train[Y_mnist_train == x[0]].shape[0])]
	X_test[i,28*28:] = X_mnist_train[Y_mnist_train == x[1]][np.random.randint(X_mnist_train[Y_mnist_train == x[1]].shape[0])]	

	Y_test[i] = np.logical_xor(x[0], x[1])

tm = MultiClassTsetlinMachine(clauses, T, s, hierarchy_structure=((tm.AND_GROUP, 28*28), (tm.OR_ALTERNATIVES, 4), (tm.AND_GROUP, 2)))
